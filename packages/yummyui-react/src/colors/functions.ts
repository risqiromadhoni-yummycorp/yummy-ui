...
export const generateForegroundColorFrom = (input: any, percentage = 0.8) =>  {
...
export const convertToHsl = (input: any) => {
  let resultObj = {};
...
export const injectThemes = (addBase, config, themes) => {
  let includedThemesObj = new Object();

  // add light themes
  if (config("yummyui.themes") == false) {
  ...
  // add custom themes
  if (Array.isArray(config("yummyui.themes"))) {
    config("yummyui.themes").forEach((item, index) => {
  ...
  let themeOrder = [];
  if (Array.isArray(config("yummyui.themes"))) {
    config("yummyui.themes").forEach((theme, index) => {
  ...
  } else if (config("yummyui.themes") != false) {
    themeOrder = [
      "light",
      "dark",
    ];
  ...
  themeOrder.forEach((themeName, index) => {
    if (index === 0) {
      ...
      // auto dark
      if (config("yummyui.darkTheme")) {
        if (
          themeOrder[0] != config("yummyui.darkTheme") &&
          themeOrder.includes(config("yummyui.darkTheme"))
        ) {
          addBase({
            ["@media (prefers-color-scheme: dark)"]: {
              [":root"]:
                includedThemesObj[
                `[data-theme=${config("yummyui.darkTheme")}]`
                ],
            },
          });
        }
